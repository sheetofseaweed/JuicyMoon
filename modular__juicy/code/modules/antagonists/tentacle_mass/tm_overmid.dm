...
GLOBAL_LIST_EMPTY(tentacle_blobs) //complete list of all blobs made.
GLOBAL_LIST_EMPTY(tentacle_blob_cores)
GLOBAL_LIST_EMPTY(tentacle_overminds)
GLOBAL_LIST_EMPTY(tentacle_blob_nodes)


/mob/camera/blob/tentacle_mass
	name = "Tentacle Overmind"
	...
	hud_type = /datum/hud/blob_overmind/tentacle_mass

/mob/camera/blob/tentacle_mass/Initialize(mapload, starting_points = 60)
	. = ..()
...
/mob/camera/blob/proc/is_valid_turf(turf/T)
	...
	return TRUE
*/
/mob/camera/blob/tentacle_mass/process()
	if(!blob_core)
		if(!placed)
...
/mob/camera/blob/Move(NewLoc, Dir = 0)
	if(placed)
		...
		return TRUE
*/
/mob/camera/blob/tentacle_mass/mind_initialize()
	. = ..()
	var/datum/antagonist/blob/B = mind.has_antag_datum(/datum/antagonist/blob)
	if(!B)
