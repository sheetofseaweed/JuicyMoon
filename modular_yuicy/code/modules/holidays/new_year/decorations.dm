...
/*/obj/item/decoration
	...
	icon = 'modular_yuicy/icons/holidays/new_year/decorations.dmi'
	icon_state = "santa"
	layer = 4.1
...
/obj/item/decoration/garland
	...
	var/random = TRUE
	var/variations = 3
	//glow_icon = 'modular_yuicy/icons/holidays/new_year/decorations.dmi'
...
/obj/item/decoration/tinsel
	...
	icon = 'modular_yuicy/icons/holidays/new_year/tinsel.dmi'
	icon_state = "1"
	var/variations = 20
	var/random = TRUE
...
/obj/item/decoration/snowflake/Initialize(mapload)
	. = ..()
...
/obj/item/device/flashlight/lamp/fir/special/Initialize(mapload)
	. = ..()
...
/obj/item/device/flashlight/lamp/fir/special/alternative
	icon = 'modular_yuicy/icons/obj/flora/pinetrees.dmi'
	icon_state = "pine_c"
...
/obj/item/device/flashlight/fir_small
	name = "small fir"
	desc = "Маленькая ёлочка."
	icon = 'modular_yuicy/icons/holidays/new_year/decorations.dmi'
	icon_state = "small_christmas_tree"
	var/icon_state_off = "small_christmas_tree"
	//brightness = 2
	light_power = 0.6
...
/obj/item/device/flashlight/fir_small/proc/update_fir()
	if(on)
		icon_state = "[icon_state_off]-on"
		set_light(2)
	else
		icon_state = "[icon_state_off]"
		set_light(0)

/obj/item/device/flashlight/fir_small/attack_self(mob/user)
	. = ..()
	if(do_after(user, 5, target = src))
		toggle()

/obj/item/device/flashlight/fir_small/verb/toggle()
	set name = "Toggle garland"
	set category = "Object"
	set src in view(1)
